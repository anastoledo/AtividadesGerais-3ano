...
HTML;
  file_put_contents("sistema/view/{$nomeTabela}.php", $conteudo); // Exemplo salvando como arquivo
        ...
        foreach ($this->tabelas as $tabela) {
             ...
             foreach($atributos as $atributo){
               if($atributo->Key=="PRI")
                    $id = "{\$dado['{$atributo->Field}']}"; // se atributo na posição for chave primaria, salva o valor na variável $id
                $attr .= "echo \"<td>{\$dado['{$atributo->Field}']}</td>\";\n"; // o resto pega os atributos normais, exibe na tabela
              }
            $conteudo="";
            $conteudo = <<<HTML
...
<html>
   ...
   foreach(\$dados as \$dado){
      echo "<tr>";
      {$attr}
      echo "<td><a href='../control/{$nomeTabela}Control.php?a=2&id={$id}'>Excluir</a></td>";  //esse a após o ? é o que define a ação no controller
