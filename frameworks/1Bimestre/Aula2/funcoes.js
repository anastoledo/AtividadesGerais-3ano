...
function configEstiloLinks() {
  ...
  estiloLinks = document.querySelector(
    'input[name="estiloLinks"]:checked'
  ).value;
  ctxLinks = "a{\n color:" + corLink + ";\n";
...
function configHtmlLinks() {
  ...
  for (let i = 0; i < links.length; i++) {
    ...
    ctxLinks +=
      '<a href="' + href[href.length - 1] + '">' + links[i].value + "</a>";
  }
  return ctxLinks;
}
...
function configHTMLConteudo() {
  ctxConteudo = "";
  let htmlConteudo = document.querySelector("#txtConteudo").value;

  let arquivoImagem = document.getElementById("arquivoImagem").value;
  if (arquivoImagem) {
    arquivoImagem = arquivoImagem.split("\\").pop();
    htmlConteudo += `<figure>
            <img src="${arquivoImagem}" alt="NÃO FUNFOU SORRY">
        </figure>\n`;
  }
